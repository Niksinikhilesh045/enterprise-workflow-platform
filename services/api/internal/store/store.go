...
type Store interface {
	...
	ListRecords(tenantID, state string) ([]domain.Record, error)
...
func (s *MemoryStore) CreateRecord(tenantID, key string, r domain.Record) (domain.Record, bool, error) {
	...
	s.appendAuditLocked(r, "record.created")
	s.appendOutboxLocked(r, "record.created")
	return r, false, nil
}

func (s *MemoryStore) ListRecords(tenantID, state string) ([]domain.Record, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *MemoryStore) UpdateRecord(tenantID, id string, expectedVersion int64, state string, data map[string]any) (domain.Record, error) {
	...
	s.records[id] = r
	s.appendAuditLocked(r, "record.updated")
	s.appendOutboxLocked(r, "record.updated")
	return r, nil
}
...
func (s *MemoryStore) appendAuditLocked(r domain.Record, typ string) {
	s.audit = append(s.audit, domain.AuditEvent{ID: s.nextID("evt"), TenantID: r.TenantID, RecordID: r.ID, Type: typ, OccurredAt: time.Now().UTC(), Payload: map[string]any{"state": r.State, "version": r.Version}})
}

func (s *MemoryStore) appendOutboxLocked(r domain.Record, typ string) {
	s.outbox = append(s.outbox, domain.OutboxEvent{ID: s.nextID("out"), TenantID: r.TenantID, AggregateID: r.ID, Topic: RecordEventsTopic, Key: r.ID, Type: typ, Payload: map[string]any{"recordId": r.ID, "workflowId": r.WorkflowID, "state": r.State, "version": r.Version}, CreatedAt: time.Now().UTC()})
}
